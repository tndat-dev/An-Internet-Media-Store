...
check "Kafka topic CRs Ready" "$(kubectl -n production get kafkatopics.kafka.strimzi.io -l strimzi.io/cluster=aims-kafka -o json | jq '[.items[] | select(.status.conditions[] | .type == "Ready" and .status == "True")] | length')" 7
check "Versioned Kafka event topics Ready" "$(kubectl -n production get kafkatopics.kafka.strimzi.io -o json | jq '[.items[] | select(.metadata.name == "aims.business.order.created.v1" or .metadata.name == "aims.business.order.lifecycle.v1" or .metadata.name == "aims.business.inventory.reserved.v1" or .metadata.name == "aims.business.inventory.rejected.v1" or .metadata.name == "aims.business.payment.completed.v1") | select(.status.conditions[] | .type == "Ready" and .status == "True")] | length')" 5
